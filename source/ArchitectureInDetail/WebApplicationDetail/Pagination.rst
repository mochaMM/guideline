...
| ごれまでは共通ライブラリから提供しているJSPタグライブラリ( \ ``<t:pagination>``\ )を使用し、GET(URL)で検索条件や :ref:`ページ検索用のリクエストパラメータ <pagination_overview_pagesearch_requestparameter>` を引き継ぐ方法について説明してきたが、別の方法で検索条件やページ検索用のリクエストパラメータを引き継ぐことができる。その方法については以下に示す。
