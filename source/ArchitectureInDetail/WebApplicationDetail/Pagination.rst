 ...
 * HTTP GETメソッドを用いた方法
   ...
   * **メリット**
     ...
     * | ページネーション対象のページをブックマークの保存及び共有ができる。

   * **デメリット**

     * | 詳細画面など、一度ページネーション対象のページから離れて、そのあとページネーション対象のページへ戻るような動作を実現しようとすると実装が複雑になる。
     ...
     * | 検索条件の項目が非常に多い場合はブラウザのURL最大文字数制約により、正しく検索条件を引き継げない可能性がある。例えば \ `Internet Explorerは2,083文字以上のURLに対応していない <https://support.microsoft.com/ja-jp/kb/208427>`_\ 。
 ...
 * HTTP POSTメソッドを用いた方法
   ...
   * **メリット**
     ...
     * | 検索条件の項目が非常に多い場合でも正しく検索条件を引き継ぐことができる。
 ...
 * HTTP POSTメソッドとセッションを用いた方法
   ...
   * **メリット**
     ...
     * | セッションに格納した検索条件やページ検索用のリクエストパラメータを使用することで詳細画面など、一度ページネーション対象のページから離れて、そのあとページネーション対象のページへ戻るようなことができる。
     * | 検索条件にセンシティブな情報（ID、メールアドレス、名前、住所、電話番号など）が含まれる場合、検索条件が露出することを防げる。
     * | 検索条件の項目が非常に多い場合でも正しく検索条件を引き継ぐことができる。
   ...
   * **デメリット**
     ...
     * | ページネーション対象のページを、複数のウインドウやタブで立ち上げた場合、互いの操作がセッション上に格納しているページ検索条件に干渉しあうため、データ整合性を保つことができなくなる。

| 本ガイドラインでは、安易にセッションにデータを格納するのではなく、まずはセッションを使わない方針で検討し、本当に必要なデータのみセッションに格納することを推奨している。詳細は、「\ :doc:`SessionManagement`\」を参照されたい。
| 特別なアプリケーションの仕様及びシステム要件以外は、HTTP POSTメソッドとセッションを用いた検索条件やページ検索用のリクエストパラメータの引き継ぎ方法は使用しない方針で検討すること。
...
| 処理のフローは、以下の通りである。
 ...
 .. list-table::
    ...
    * - 項番
      - 説明
    * - | (1)
      - | 検索一覧画面の検索ボタン及びページリンクを押下した際の検索処理を行う。

        * 検索条件の項目をセッションから受け取る。
        * ページリンクのアンカータグで指定したページ検索用のリクエストパラメータを受け取る。
        * 検索条件の項目とページ検索用のリクエストパラメータを使用して検索処理を行う。
        * ページ検索用のリクエストパラメータをセッションに格納する。

    * - | (2)
      - | 詳細画面から戻るボタンを押下した際のページネーション対象のページへ戻る処理を行う。

        * 検索条件の項目とページ検索用のリクエストパラメータをセッションから受け取る。
        * セッションから受け取った、検索条件の項目とページ検索用のリクエストパラメータを使用して検索処理を行う。
        * ページ検索用のリクエストパラメータをセッションに格納する。
