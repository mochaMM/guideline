...
* マッピングファイルにSQLを定義する。
 ...
 .. warning:: **fetchSize属性の指定について**
    ...
    \ ``fetchSize``\は、JDBCドライバとデータベース間の１回の通信で取得するデータの件数を設定するパラメータである。
    なお、terasoluna-gfw-mybatis3 5.2.0.RELEASE以降でサポートしているMyBatis 3.3.0以降のバージョンでは、
...
terasoluna-gfw-mybatis3 5.2.0.RELEASE以降でサポートしているMyBatis 3.3.0以降のバージョンでは、
...
terasoluna-gfw-mybatis3 5.2.0.RELEASE以降でサポートしているMyBatis 3.3.0以降のバージョンではJAVASSISTがデフォルトで使用される。
