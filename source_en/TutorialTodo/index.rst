...
.. list-table::
   ...
   * - | (3)
     - | Carry out the settings for accessing the static resource (css, images, js etc.).
       ...
       | ``cache-period="3600"`` is also correct, however, in order to demostrate that it is 60 minutes, it is better to write as ``cache-period="#{60 * 60}"`` which uses `SpEL <http://static.springsource.org/spring/docs/3.2.18.RELEASE/spring-framework-reference/html/expressions.html#expressions-beandef-xml-based>`_ .
   * - | (4)
